            ...
            {selectedResult ? (
              <div className="bg-white rounded-lg shadow-lg p-8">
                <div className="mb-8">
                  ...
                  {/* Geri Bildirim */}
                  <div className="bg-gray-50 rounded-lg p-6 border-l-4 border-blue-500 mb-8">
                  ...
                  {selectedResult.detailedAnalysis && selectedResult.detailedAnalysis.length > 0 && (
                    <div className="bg-white rounded-lg p-6 border-l-4 border-green-500 mb-8">
                      <h3 className="text-2xl font-bold text-gray-800 mb-6">
                        🔍 Her Sorunun Detaylı Analizi
                      </h3>
                      <div className="space-y-8">
                        {selectedResult.detailedAnalysis.map((analysis: any, index: number) => (
                          <div key={index} className="bg-gray-50 rounded-lg p-6 border-l-4 border-purple-500">
                            <div className="mb-4">
                              <h4 className="text-lg font-bold text-gray-800 mb-3">
                                📌 Soru {analysis.questionNumber}
                              </h4>
                              
                              <div className="flex flex-col gap-3 mb-4">
                                <div>
                                  <p className="font-semibold text-gray-700">🏷️ Konu:</p>
                                  <p className="text-gray-600">{analysis.topic}</p>
                                </div>
                                <div>
                                  <p className="font-semibold text-gray-700">❓ Soru:</p>
                                  <p className="text-gray-600">{analysis.question}</p>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    <p className="font-semibold text-red-700">❌ Seçilen:</p>
                                    <p className="text-red-600">{analysis.userAnswer}</p>
                                  </div>
                                  <div>
                                    <p className="font-semibold text-green-700">✅ Doğru Cevap:</p>
                                    <p className="text-green-600">{analysis.correctAnswer}</p>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="bg-blue-50 rounded-lg p-4 border-l-4 border-blue-400">
                                <p className="font-semibold text-blue-900 mb-2">💡 AI Tavsiyesi:</p>
                                <p className="text-gray-700 text-sm leading-relaxed whitespace-pre-wrap">{analysis.aiAdvice}</p>
                              </div>
                            </div>
                          </div>
                        ))}
